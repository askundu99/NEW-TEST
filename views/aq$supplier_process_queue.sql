--
-- View "AQ$SUPPLIER_PROCESS_QUEUE"
--
CREATE OR REPLACE FORCE EDITIONABLE VIEW "XX_TANGO"."AQ$SUPPLIER_PROCESS_QUEUE" ("QUEUE", "MSG_ID", "CORR_ID", "MSG_PRIORITY", "MSG_STATE", "DELAY", "DELAY_TIMESTAMP", "EXPIRATION", "ENQ_TIME", "ENQ_TIMESTAMP", "ENQ_USER_ID", "ENQ_TXN_ID", "DEQ_TIME", "DEQ_TIMESTAMP", "DEQ_USER_ID", "DEQ_TXN_ID", "RETRY_COUNT", "EXCEPTION_QUEUE_OWNER", "EXCEPTION_QUEUE", "USER_DATA", "PROPAGATED_MSGID", "SENDER_NAME", "SENDER_ADDRESS", "SENDER_PROTOCOL", "ORIGINAL_MSGID", "ORIGINAL_QUEUE_NAME", "ORIGINAL_QUEUE_OWNER", "EXPIRATION_REASON", "CONSUMER_NAME", "ADDRESS", "PROTOCOL") DEFAULT COLLATION "USING_NLS_COMP"  AS 
  with msgbm as  (select 'SUPPLIER_PROCESS_QUEUE' QUEUE_NAME, qt_mc.msgid MSGID, qt_mc.correlation CORRELATION, qt_mc.priority PRIORITY, qt_mc.state STATE, qt_mc.delivery_time DELIVERY_TIME, qt_mc.expiration EXPIRATION,  qt_mc.enq_time ENQ_TIME, qt_mc.bitmap SBITMAP,  qt_mc.lck_bitmap LCK_BITMAP  from TABLE(AQ$_GET_CACHE_MSGBM(98300))  qt_mc where qt_mc.state NOT IN (0, 1, 2, 128, 192 ))  SELECT 
